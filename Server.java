...
public class Server extends Thread{
     ...
     */
     ...
     {   int accIndex;             	/* Index position of account to update */
         ...
         { 
        	 while( (objNetwork.getInBufferStatus().equals("empty"))) { //deadlock: client is disconnected before this while loop is tested.
        		 if(objNetwork.getClientConnectionStatus().equals("Idle")||objNetwork.getClientConnectionStatus().equals("disconnected")) break;//no more to take
        		 System.out.println("DEBUG: class SEVER: InBuffer is empty, nothing to take..");
        		 Thread.yield();//waiting
        		 System.out.println("DEBUG: SEVER - Re-checking the In-buffer");
        	 ...
        	 if (!objNetwork.getInBufferStatus().equals("empty"))
        	 {
        		 ...
        		 while( (objNetwork.getOutBufferStatus().equals("full"))) {
        			 System.out.println("DEBUG: SEVER IS WAITING - OutBuffer is full");
        			 Thread.yield();
        			 System.out.println("DEBUG: SEVER - RE-sending to Outbuffer");
    ...
    {   Transactions trans = new Transactions();
        ...
        if(objNetwork.getClientConnectionStatus().equals("Idle")) objNetwork.setServerConnectionStatus("Idle");
        else if(objNetwork.getClientConnectionStatus().equals("disconnected")) objNetwork.disconnect(objNetwork.getServerIP());//disconnect the sever after client is disconnected.
